...
function Shell({
  ...
  workspace: "user" | "monitor";
  setWorkspace: (workspace: "user" | "monitor") => void;
    ...
    <div className="pam-app">
      ...
      <div className="pam-body">
        ...
        <main className="pam-work">
          ...
          <section className={`pam-frame ${workspace === "monitor" ? "monitor-frame" : ""}`}>
...
const digitalEmployeeStages = [
  { title: "议案收集与整理", desc: "", x: 180, lane: "收集整理" },
  { title: "议案数据审核", desc: "", x: 447, lane: "审核校验" },
  { title: "审议流程管理", desc: "", x: 714, lane: "审议决策" },
  { title: "决议分发", desc: "", x: 981, lane: "决议分发" },
  { title: "决议/指令执行监督", desc: "", x: 1249, lane: "执行监督" },
  { title: "议案归档", desc: "", x: 1516, lane: "归档沉淀" },
] as const;

const digitalEmployeeNodes = [
  { id: "01", title: "议案收集", x: 111, y: 422, tone: "cyan", lane: 0, steps: ["卓越议案收集", "门户议案收集", "钉钉议案收集", "议案路由"] },
  { id: "02", title: "议案信息整理", x: 111, y: 558, tone: "cyan", lane: 0, steps: ["抽取关键字段到元信息并判断完整性", "驳回修改"] },
  { id: "03", title: "完整性检查", x: 378, y: 374, tone: "indigo", lane: 1, steps: ["完整性规则匹配与校验", "完整性驳回修改并判断修改处", "发给战执委判断"] },
  { id: "04", title: "基础性检查", x: 378, y: 490, tone: "indigo", lane: 1, steps: ["基础性规则匹配与校验", "基础性驳回修改并判断修改处", "发给战执委判断"] },
  { id: "05", title: "职能相关检查", x: 378, y: 606, tone: "indigo", lane: 1, steps: ["职能相关规则匹配与校验", "职能驳回修改并判断修改处", "发给职能人员判断"] },
  { id: "06", title: "战执委相关检查", x: 378, y: 722, tone: "indigo", lane: 1, steps: ["战执委相关规则匹配与校验", "战执委驳回修改并判断修改处", "发给职能人员判断"] },
  { id: "07", title: "生成审议材料", x: 645, y: 374, tone: "violet", lane: 2, steps: ["审议材料整理", "通知战执委确认/发送审议信息"] },
  { id: "08", title: "进行审议监控", x: 645, y: 522, tone: "violet", lane: 2, steps: ["投票审议监控", "线上审议监控", "线下审议监控"] },
  { id: "09", title: "生成审议总结", x: 645, y: 670, tone: "violet", lane: 2, steps: ["生成审议公告", "生成决议", "生成指令", "通知战执委确认/发送审议公告", "通知战执委确认决议", "通知战执委确认指令"] },
  { id: "10", title: "决议文件接收并审批", x: 912, y: 422, tone: "amber", lane: 3, steps: ["决议文件接收", "决议文件向上审批"] },
  { id: "11", title: "决议文件路由通知", x: 912, y: 558, tone: "amber", lane: 3, steps: ["决议文件横向路由", "决议文件向下路由", "通知相关部门备案", "通知发起部门并指定负责人"] },
  { id: "12", title: "执行监听", x: 1179, y: 422, tone: "green", lane: 4, steps: ["问询查询", "执行结果提交", "问询并返回当前状态", "发送模板并收集", "执行结果分析"] },
  { id: "13", title: "执行归档", x: 1179, y: 558, tone: "green", lane: 4, steps: ["整理归档资料", "通知战执委", "通知归档人员"] },
  { id: "14", title: "整理归档材料", x: 1446, y: 422, tone: "rose", lane: 5, steps: ["整理归档材料", "分析执行结果"] },
  { id: "15", title: "通知监督人", x: 1446, y: 558, tone: "rose", lane: 5, steps: ["通知监督人"] },
] as const;

const digitalEmployeeLanes = ["收集整理", "数据审核", "审议流程", "决议分发", "执行监督", "议案归档"] as const;
...
function DigitalEmployeeFlow({ notice }: { notice: (s: string) => void }) {
  ...
  const crossLinks = nodesByLane.slice(0, -1).map((nodes, index) => {
    const next = nodesByLane[index + 1];
    const from = nodes[nodes.length - 1];
    const to = next[0];
    return `M${from.x + 138} ${from.y + 35} C${from.x + 196} ${from.y + 35} ${to.x - 58} ${to.y + 35} ${to.x} ${to.y + 35}`;
  });
  const requestedCrossLinks = [
    "M249 457 C303 457 318 409 378 409",
    "M783 705 C940 705 1019 457 1179 457",
  ];
  ...
  const completedTraceRoute = (group: DigitalEmployeeNode) => new Set(({
    "01": [0, 3], "08": [0], "09": [0, 3], "11": [0, 2], "12": [0, 2, 4],
  ...
  const traceLinkPath = (from: { x: number; y: number }, to: { x: number; y: number }) => {
    if (Math.abs(to.y - from.y) < 28) {
      ...
      const y = from.y + 27;
      return `M${startX} ${y} C${startX + (rightwards ? 36 : -36)} ${y - 8} ${endX + (rightwards ? -36 : 36)} ${to.y + 27 + 8} ${endX} ${to.y + 27}`;
    }
    const startX = from.x + 69;
    const endX = to.x + 69;
    return `M${startX} ${from.y + 54} C${startX + 28} ${from.y + 80} ${endX - 34} ${to.y - 24} ${endX} ${to.y}`;
  };
  ...
  const nodeData = selectedTraceNode ? {
    input: [
      { key: "当前议案", type: "text", value: "技术改造项目议案 · 战略执行委员会提报" },
      { key: "待处理材料", type: "file", value: "《技术改造项目议案》申报材料.pdf · 2.4 MB" },
      { key: "关键业务信息", type: "table", value: "提案编号 / 归口部门 / 预算金额 / 提交日期" },
    ],
    output: [
      { key: "本环节处理结果", type: "text", value: `已完成「${selectedTraceNode}」处理，结果已写入议案流转记录。` },
      { key: "处理说明", type: "file", value: "议案处理说明.docx · 18 KB" },
      { key: "业务核查结论", type: "table", value: "核查事项 / 核查结论 / 后续处理意见" },
    ],
  } : null;
  const renderNodeRecord = (record: { key: string; type: string; value: string }) => {
    const columns = record.value.split(" / ");
    const tableRow = record.key === "关键业务信息" ? ["2026-001", "战略执行委员会", "¥ 2,400,000", "2026-08-18"] : ["材料完整", "无重大风险", "进入下一环节"];
    return <div className={`bp-data-record ${record.type}`} style={{ gridTemplateColumns: "minmax(96px, .9fr) minmax(0, 1.45fr)" }} key={record.key}><code style={{ borderRight: "1px solid rgba(177, 197, 255, .18)", paddingRight: 9 }}>{record.key}</code>{record.type === "table" ? <div className="bp-data-table" style={{ gridColumn: "auto", margin: 0 }}><table><thead><tr>{columns.map((column) => <th key={column}>{column}</th>)}</tr></thead><tbody><tr>{tableRow.map((cell, index) => <td key={`${cell}-${index}`}>{cell}</td>)}</tr></tbody></table></div> : record.type === "file" ? <div className="bp-data-file"><FileText size={17}/><div><b>{record.value.split(" · ")[0]}</b><small>{record.value.split(" · ")[1]}</small></div><i>↗</i></div> : <p>{record.value}</p>}</div>;
  };
  const nodeDataModal = selectedTraceNode && nodeData && <div className="bp-node-data-modal-backdrop" onClick={() => setSelectedTraceNode(null)}><section className="bp-node-data-modal" role="dialog" aria-modal="true" aria-label={`${selectedTraceNode} 节点数据`} onClick={(event) => event.stopPropagation()}><header><div><small>BUSINESS PROCESS DETAIL</small><h3>{selectedTraceNode}</h3></div><button type="button" onClick={() => setSelectedTraceNode(null)} aria-label="关闭数据"><X size={17}/></button></header><section><h4>输入信息</h4>{nodeData.input.map(renderNodeRecord)}</section><section><h4>输出结果</h4>{nodeData.output.map(renderNodeRecord)}</section><footer><div><span>SKILL</span><b>{traceTool(selectedTraceNode)}</b></div><div><span>MCP</span><b>{traceMcp(selectedTraceNode)}</b></div></footer></section></div>;
      ...
      <div className="blueprint-canvas" style={{ transform: `translate(${pan.x}px, ${pan.y}px) scale(${zoom})` }}>
        <svg className="blueprint-lines" viewBox="0 0 1735 840" aria-hidden="true">
          ...
          <g className="bp-stage-links">
            {[362, 629, 897, 1164, 1432].map((x) => { const path = `M${x} 166 H${x + 78}`; return <g key={x}><path d={path} markerEnd="url(#bp-stage-arrow)" /><circle className="bp-moving-dot" r="2.3"><animateMotion dur="1.8s" repeatCount="indefinite" path={path} /></circle></g>; })}
          </g>
          <g className="bp-stage-maps">
            {[180, 447, 714, 981, 1249, 1516].map((x) => { const path = `M${x} 186 V366`; return <path d={path} markerEnd="url(#bp-down-arrow)" key={x} />; })}
          </g>
          ...
          <g className="bp-cross-flow" filter="url(#bp-glow)">
            ...
            {requestedCrossLinks.map((path) => <g key={path}><path d={path} markerEnd="url(#bp-main-arrow)"/><circle className="bp-moving-dot" r="2.1"><animateMotion dur="2.7s" repeatCount="indefinite" path={path} /></circle></g>)}
          </g>
        ...
        {digitalEmployeeNodes.map((node) => <button type="button" className={`bp-node ${node.tone}`} style={{ left: node.x, top: node.y }} key={node.id} onClick={() => { setSelectedNode(node); setSelectedTraceNode(null); setDrawerView("tasks"); }}><em>二级流程 {node.id}</em><i>{node.id}</i><b>{node.title}</b><span>↗</span></button>)}
      </div>
...
function App() {
  ...
  const [workspace, setWorkspace] = useState<"user" | "monitor">(
    monitorPages.has(activePage as never) ? "monitor" : "user",
  );
  useEffect(() => {
    const nextWorkspace = monitorPages.has(activePage as never) ? "monitor" : "user";
  ...
  const switchWorkspace = (next: "user" | "monitor") => {
    setWorkspace(next);
    const pages = next === "user" ? userPages : monitorPages;
    if (!pages.has(page as never)) {
      if (next === "user") setPage("lifecycle");
      else setPage("digital-employee-flow");
    }
  };
